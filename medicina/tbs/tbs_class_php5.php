...
------------------------
Version  : 3.3.0 for PHP 5 (there is another edition for PHP 4)
Date     : 2008-02-29
...
// Check if an expression like "exrp1=expr2" is true or false.
	...
	$Nude1 = $this->f_Misc_DelDelimiter($Val1,'\'');
	$Val2  = trim(substr($Str,$Pos+$Len));
	$Nude2 = $this->f_Misc_DelDelimiter($Val2,'\'');
			...
			if ($BegCnt===0) {
				...
				if (($Chr===' ') or ($Chr==="\r") or ($Chr==="\n")) {
					if ($Status===1) {
						...
						$this->f_Loc_PrmCompute($Txt,$Loc,$SubName,$Status,$HtmlTag,$DelimChr,$DelimCnt,$PosName,$PosNend,$PosVal,$Pos);
						$Status = 0;
					}
...
function f_Loc_PrmCompute(&$Txt,&$Loc,&$SubName,$Status,$HtmlTag,$DelimChr,$DelimCnt,$PosName,$PosNend,$PosVal,$Pos) {
	...
	if ($Status===0) {
		...
		if ($SubName) {
			...
			if ($x==='if') {
				$this->f_Loc_PrmIfThen($Loc,true,$v);
			} elseif ($x==='then') {
				$this->f_Loc_PrmIfThen($Loc,false,$v);
			} else {
				$Loc->PrmLst[$x] = $v;
			}
		}
...
//Modify $Loc, return false if tags not found, returns the inner source of tag if $RetInnerSrc=true
  ...
  for ($i=0;$i<=$TagMax;$i++) {
	...
	$TagO = $this->f_Xml_FindTag($Txt,$TagLst[$Ref],true,$Loc->PosBeg-1,false,$LevelStop,false);
	...
	if ($TagIsSgl[$Ref]) {
		...
		$TagC = $this->f_Xml_FindTag($Txt,$TagLst[$Ref],false,$Loc->PosEnd+1,true,-$LevelStop,false);
	...
	for ($i=$Ref+1;$i<=$TagMax;$i++) {
		...
		if (($x!=='') and ($TagC!==false)) {
			...
			$TagC = $this->f_Xml_FindTag($Txt,$x,$TagIsSgl[$i],$PosEnd+1,true,$level,false);
	...
	for ($i=$Ref-1;$i>=0;$i--) {
		...
		if (($x!=='') and ($TagO!==false)) {
			$level = ($TagIsSgl[$i]) ? 1 : 0;
			$TagO = $this->f_Xml_FindTag($Txt,$x,true,$PosBeg-1,false,$level,false);
...
function f_Xml_GetPart(&$Txt,$Tag,$WithTags=false,$CancelIfEmpty=false) {
...
// The $CancelIfEmpty parameter enables to cancel the extraction when the part is not found.
	...
	$LocOpen = $this->f_Xml_FindTag($Txt,$Tag,true,0,true,false,false);
	if ($LocOpen!==false) {
		$LocClose = $this->f_Xml_FindTag($Txt,$Tag,false,$LocOpen->PosEnd+1,true,false,false);
		if ($LocClose!==false) {
			if ($WithTags) {
...
*/
	...
	if ($TagOk) {
		...
		if ($WithPrm) {
			$PosEnd = 0;
			$this->f_Loc_PrmRead($Txt,$PosX,true,'\'"','<','>',$Loc,$PosEnd);
		} else {
