<?PHP
 ...
 elseif ($_REQUEST['cad_p_bas'] == '62S'){
        if ($_REQUEST['nome_p_bas'] != ''){
            $sql_pesq_basico = "SELECT C4_ID AS ID, C4_DESCR AS DESCR FROM SC401 WHERE UPPER(C4_DESCR) LIKE'%".strtoupper($_REQUEST['nome_p_bas'])."%' ORDER BY C4_DESCR";}
    elseif ($_REQUEST['id_p_bas'] != '') {
            $sql_pesq_basico = "SELECT C4_ID AS ID, C4_DESCR AS DESCR FROM SC401 WHERE C4_ID =".$_REQUEST['id_p_bas']." ORDER BY C4_DESCR";}
    else   {$sql_pesq_basico = "SELECT C4_ID AS ID, C4_DESCR AS DESCR FROM SC401 WHERE C4_DESCR LIKE'%%' ORDER BY C4_DESCR";};
      }
 elseif ($_REQUEST['cad_p_bas'] == '63S'){
        if ($_REQUEST['nome_p_bas'] != ''){
            $sql_pesq_basico = "SELECT C1_ID AS ID, C1_DESCR AS DESCR FROM SC101 WHERE UPPER(C1_DESCR) LIKE'%".strtoupper($_REQUEST['nome_p_bas'])."%' ORDER BY C1_DESCR";}
    elseif ($_REQUEST['id_p_bas'] != '') {
            $sql_pesq_basico = "SELECT C1_ID AS ID, C1_DESCR AS DESCR FROM SC101 WHERE C1_ID =".$_REQUEST['id_p_bas']." ORDER BY C1_DESCR";}
    else   {$sql_pesq_basico = "SELECT C1_ID AS ID, C1_DESCR AS DESCR FROM SC101 WHERE  C1_DESCR LIKE'%%' ORDER BY C1_DESCR";};
      }
 elseif ($_REQUEST['cad_p_bas'] == '64S'){
        if ($_REQUEST['nome_p_bas'] != ''){
            $sql_pesq_basico = "SELECT C2_ID AS ID, C2_DESCR AS DESCR FROM SC201 WHERE UPPER(C2_DESCR) LIKE'%".strtoupper($_REQUEST['nome_p_bas'])."%' ORDER BY C2_DESCR";}
    elseif ($_REQUEST['id_p_bas'] != '') {
            $sql_pesq_basico = "SELECT C2_ID AS ID, C2_DESCR AS DESCR FROM SC201 WHERE C2_ID =".$_REQUEST['id_p_bas']." ORDER BY C2_DESCR";}
    else   {$sql_pesq_basico = "SELECT C2_ID AS ID, C2_DESCR AS DESCR FROM SC201 WHERE C2_DESCR LIKE'%%' ORDER BY C2_DESCR";};
      };
